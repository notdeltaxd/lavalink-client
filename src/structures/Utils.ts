...
export class ManagerUtils {
  ...
  validateQueryString(
    ...
    if (
      this.LavalinkManager.options?.autoChecks?.sourcesValidations &&
      !node.info.sourceManagers?.length
    )
      ...
      if (
        this.LavalinkManager.options?.linksBlacklist.some(
          (v) =>
            ...
            (isRegExp(v) && (v as RegExp).test(queryString))
        )
      ) {
        throw new Error(
    ...
    if (
      ...
      throw new Error(
        "Query / Link Provided for this Source but Lavalink Node has not 'pandora' enabled"
      );
    }
    if (
      ...
      throw new Error(
        "Query / Link Provided for this Source but Lavalink Node has not 'gaana' enabled"
      );
    }
    if (
  ...
  validateSourceString(node: LavalinkNode, sourceString: SearchPlatform) {
    ...
    if (
      source === "speak" &&
      this.LavalinkManager.options?.autoChecks?.pluginValidations &&
    ...
    if (
      ["pdsearch", "pdisrc", "pdrec"].includes(source) &&
      !node.info?.sourceManagers?.includes("pandora")
    ) {
      throw new Error(
        "Lavalink Node has not 'pandora' enabled, which is required to have '" +
          source +
          "' work"
      );
    }
    if (
      ...
      throw new Error(
        "Lavalink Node has not 'gaana' enabled, which is required to have 'gaanasearch' work"
      );
    }
    if (
      source === "amznsearch" &&
